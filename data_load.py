...
class Loader(Dataset):
	...
	def __init__(self, hdf5_clean, hdf5_attack, n_cycles=1):
	...
	def __getitem__(self, index):
		...
		return torch.from_numpy(fv_clean).float().contiguous(), torch.from_numpy(fv_attack).float().contiguous(), torch.zeros(1), torch.ones(1)
